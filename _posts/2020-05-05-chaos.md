...
Let $$X\subseteq$$ $$$\mathbb{R}^{m}$$$ and let $$B\subseteq$$ $$\R^m$$ and let B be a minimal-dimensional, minimally-sized box such
...
$$d = \lim\limits_{n \to \infty}{\frac{log(number \: of \: sub \: boxes \:  of \: B_n \: that \: intersect \: X)}{log(n)}}$$
